...
public:
    ...
    explicit Temperature(double celsius) {
        if (celsius < -273.15){
            throw invalid_argument("Celsius cannot be below the absolute zero '- 273.15'");
            //.std::cout << "Celsius cannot be below the absolute zero '- 273.15'" << endl;
        }
    ...
    void setCelsius(double celsius) {
        // TODO: Implement
        if (celsius < -273.15){
            throw std::invalid_argument("Celsius cannot be below the absolute zero '- 273.15'");
        }
        celsius_ = celsius;
    }
};
...
public:
    ...
    BankAccount(const string& owner, double initialBalance) {
        ...
        if (owner == "" || initialBalance < 0){
            throw invalid_argument("the name of the owner is not valid or the balance is less than 0");
        }
        owner_=owner;
    ...
    void deposit(double amount) {
        // TODO: Implement
        if (amount <=0){
            throw invalid_argument("Deposit amout must be possitive");
        }
        balance_+=amount;
    }
    ...
    void withdraw(double amount) {
        // TODO: Implement
        if (amount <=0){
            throw invalid_argument("Withdrawal amout must be possitive");
        }
        if (amount > balance_){
...
private:
    ...
    static void validate(const string& pwd) {
        ...
        if (!hasDigit(pwd)){
            throw invalid_argument("password must contain atleast on digit.");
        }
    }
